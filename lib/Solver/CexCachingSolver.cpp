...
struct ResponseComparator {
  bool operator()(const ref<SolverResponse> a, const ref<SolverResponse> b) const {
    return *a < *b;
  }
};
...
class CexCachingSolver : public SolverImpl {
  ...
  bool lookupResponse(const Query& query, KeyType &key, ref<SolverResponse> &result);
...
struct isInvalidResponse {
  bool operator()(ref<SolverResponse> a) const { return isa<InvalidResponse>(a); }
};
...
/// either a satisfying invalid response (for a satisfiable query), or valid response (for an
/// unsatisfiable query).
/// \return - True if a cached result was found.
bool CexCachingSolver::searchForResponse(KeyType &key, ref<SolverResponse> &result) {
  ...
  if (CexCacheTryAll) {
    ...
    // solver response. While searching subsets, we also explicitly the solutions for
...
/// \param result [out] - The cached result, if the lookup is successful. This is
/// either a InvalidResponse (for a satisfiable query), or ValidResponse (for an
/// unsatisfiable query).
/// \return True if a cached result was found.
...
bool CexCachingSolver::getResponse(const Query &query,
  ...
  if (lookupResponse(query, key, result))
    return true;
  Query query1 = query;
  query1.produceValidityCore = true;

  if (!solver->impl->check(query1, result)) {
    ...
    if (!solver->impl->computeInitialValues(query, objects, values, tmp))
      return false;
  ...
  if (CexCacheValidityCores &&
      result->getValidityCore(resultCore)) {
...
bool CexCachingSolver::computeValidityCore(const Query &query, ValidityCore &validityCore,
  ...
  isValid = isa<ValidResponse>(a);
  if (isValid)
    cast<ValidResponse>(a)->getValidityCore(validityCore);
  return true;
}
