...
public:
  ...
  struct InnerSetUnion
    ...
    void addExpr(ref<Expr> e) {
      ...
      roots.insert(e);
      rank[e] = 0;
      disjointSets[e] = new IndependentConstraintSet(e, false);
  ...
  ref<const IndependentConstraintSet> update() const;
  ref<const IndependentConstraintSet>
  updateConcretization(const Assignment &delta,
                       std::map<ref<Expr>, ref<Expr>> &changedExprs) const;
  ref<const IndependentConstraintSet>
  removeConcretization(const Assignment &delta,
                       std::map<ref<Expr>, ref<Expr>> &changedExprs) const;

  IndependentConstraintSet();
  IndependentConstraintSet(ref<Expr> e, bool toBeSplit = true);
  IndependentConstraintSet(ref<Symcrete> s, bool toBeSplit = true);
  ...
  static ref<const IndependentConstraintSet> merge(ref<const IndependentConstraintSet> a,
                                             ref<const IndependentConstraintSet> b);
